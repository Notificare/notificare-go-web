...
export default class UserController extends Controller {
  ...
  async unregisterDevice(e) {
    e.preventDefault();
    try {
      await this.notificare.registerDevice(null, null);
  ...
  async loadUserDataFields() {
    try {
      let userData = await this.notificare.fetchUserData();
  ...
  *updateUserDataFields() {
    yield timeout(500);
    try {
      yield this.notificare.updateUserData(this.sanitizeOutputValues());
    } catch (e) {}
  }
